...
rg -q 'pub fn audit_systemd_effect_store_cut' "$runtime" &&
  ...
  rg -q 'SQLITE_OPEN_NOFOLLOW' "$runtime" &&
...
rg -q 'does not acquire the live execution lock' "$contract" &&
  rg -q 'does not become an authoritative live attempt store' "$contract" ||
