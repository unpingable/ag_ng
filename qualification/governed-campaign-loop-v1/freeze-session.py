...
def strict_ssh_argv(worker: Path, port: int) -> list[str]:
    return [
        ...
        "-o", f"UserKnownHostsFile={worker / 'known_hosts'}",
...
def main() -> int:
    ...
    build_receipt = json.loads((custody / "build" / "build-receipt.json").read_text())
    ...
    devices = {
        "root": {
            **stat_identity(worker / "gcl-v1-worker-root.qcow2"),
            "sha256": sha256(worker / "gcl-v1-worker-root.qcow2"),
            "qemu_read_only": True,
        },
        "state": {
